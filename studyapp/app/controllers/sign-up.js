		...
		nextStep() {
			...
			if (this.get('currentStep') === 1) {
				...
				if (this.get('firstname').length > 0)
					properFirstName = true;
				if (this.get('lastname').length > 0)
					properLastName = true;
				if (this.get('email').replace(/.+@.+\..+/g, "").length != this.get('email').length)
					properEmail = true;
				if (this.get('password1').length > 0)
					properPassword1 = true;
				if (this.get('password2').length > 0)
					properPassword2 = true;
				...
				if (properPassword1 && properPassword2 && passwordMatch && properEmail && properFirstName && properLastName) {
					...
					if (!properPassword2)
						this.set('password2Error', true);
					if (!passwordMatch)
						this.set('passwordMatchError', true);
				}
			}
		},
		prevStep() {
